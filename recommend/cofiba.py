...
class Cofiba:
    def __init__(self, dimension, alpha, alpha_2, n, cluster_init='Erdos-Renyi'):
        ...
        self.UGraph = []
        self.Uclusters = []
        for i in range(self.N_components_Movie):
            if self.cluster_init == 'Erdos-Renyi':
                p = 3 * math.log(n) / n
    ...
    def decide(self, pool_movies, userID):
        ...
        for x in pool_movies:
            x_pta = self.users[userID].getScore(self.Uclusters[self.Mclusters[x.id]],x.featureVector,self.alpha, userID,self.users)
            if maxPTA < x_pta:
                picked = x
                maxPTA = x_pta
        self.time += 1
    ...
    def test(self,times):
        ...
        for t in range(times):
            for user in self.users:
                ...
                MovieClusterNum = self.Mclusters[picked.id]  # Get the cluster number of item
