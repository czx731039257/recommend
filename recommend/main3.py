...
if __name__== '__main__':
    tag=pd.DataFrame(pd.read_csv('ml-latest/cofiba/tag.csv'))
    tag['index'] = range(len(tag))
    tag.index = range(len(tag))

    groupby=tag.groupby(['userId','movieId'])
    i=[]

    print(tag)
    for name,group in groupby:
        if len(group)>1:
            x=group['timestamp'].idxmin()
            i.append(x)
        else:
            i.append(group.iat[0,4])
    tag=tag.loc[tag['index'].isin(i)]
    print(tag)
    tag.drop('index',axis=1,inplace=True)
    tag.index=range(len(tag))
    tag.to_csv('ml-latest/cofiba/tag1.csv',index=False,sep=',')